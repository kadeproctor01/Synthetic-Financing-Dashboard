...
export default function Home() {
  ...
  const sortedCandidates = useMemo(
    () => [...(result?.candidates ?? [])].sort((a, b) => a.financing.effectiveAnnualRateDecimal - b.financing.effectiveAnnualRateDecimal).slice(0, 8),
    [result]
  );
      ...
      <section className="workspace">
        <aside className="inputPanel">
          <div>
            ...
            <p className="subtle">Internal deterministic calculator using workbook-derived methodology and mock market data.</p>
          </div>
          ...
          <div className="method">
            <h2>Method Status</h2>
            <p>Rates are represented as decimals internally and shown as percentages. Candidate ranking is provisional: lowest all-in effective annual rate, then lowest excess proceeds.</p>
          </div>
        </aside>
          ...
          {!result ? (
            ...
            <>
              ...
              {selected ? (
                ...
                <div className="banner danger">No valid structure was produced. Review validation issues before proceeding.</div>
              )}
                  ...
                  <tbody>
                    {sortedCandidates.map((candidate) => (
                      <tr key={candidate.candidateId}>
                        <td>
                          <span className={`pill ${candidate.quoteStatus.toLowerCase()}`}>{candidate.quoteStatus}</span>
                        </td>
